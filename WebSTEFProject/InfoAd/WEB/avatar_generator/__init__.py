...
"""
    ...
    >>> photo = Avatar.generate(128, "SY", "example@sysnove.fr", "PNG")
"""
...
class Avatar(object):
    ...
    @classmethod
    def generate(cls, size, string, key, filetype="JPEG"):
        """
    ...
    def _background_color2(s):
        """
            ...
            (same seed will produce the same color).
        """
        rand_color = randomcolor.RandomColor(seed=s)
        return rand_color.generate(format_='rgb')[0]
    ...
    def _text_position(size, text, font):
        ...
        widthoffset, heightoffset = font.getoffset(text)

        left = ((size - width - widthoffset) / 2.0)
        top = ((size - height - heightoffset) / 2.0)

        """
        left = ((size - width ) / 2.0) - widthoffset
        top = ((size - height ) / 2.0) - heightoffset


        print('width : ' + str(width) + ' height : ' + str(height))
        print('widthoffset : ' + str(widthoffset) + ' heightoffset : ' + str(heightoffset))

...
for c in lines[2:]:
	nom,pre,soc,tel,pays = c.split(';')
	if '____'.join([nom,pre,soc]) in deja:
		x+=1
		continue
	deja.append('____'.join([nom,pre,soc]))
	if not (re.match('\+[0-9]{1,3}-[0-9]{9,11}', tel)):
		y+=1
	if pays not in lpays:
		z+=1
